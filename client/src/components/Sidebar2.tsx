...
const Sidebar2: React.FC<SidebarProps> = ({ darkMode, role, open, toggleSidebar }) => {
  ...
  const navItems: NavItem[] =
    role === "approver"
      ? [
          ...
          { title: "Process Request", icon: DocumentCheckIcon, path: "/request/approver" },
  ...
  const activeClass = "bg-blue-500 text-white"; // Change to your preferred active color
          ...
          <div className="gap-2 w-full">
            {navItems.map((item) => (
              <Link to={item.path} key={item.title}>
                <li
                  className={`${listStyle} ${location.pathname === item.path ? activeClass : ""} ${!open ? "justify-center" : "hover:bg-[#E7F1F9]"}`}
                >
                  <div className={`p-2 ${!open ? "hover:bg-[#D2E6F7] rounded-lg" : ""}`}>
            ...
            <div onClick={handleLogout} className="border-t flex justify-center items-center cursor-pointer">
